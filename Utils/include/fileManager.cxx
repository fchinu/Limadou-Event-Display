...
double *FileManager::getColumn(const char *columnName) const {
  ...
  while (index < FileManager::fNColumns) {
    if (FileManager::fDataNames[index] == columnName)
      return FileManager::getColumn(index);
    index++;
  }
...
/**
 * @brief Append a column to the existing dataset. This will NOT automatically
 * added to the file.
 *
 * @param column
 * @param columnName
...
void FileManager::addColumn(double *column, const char *columnName) {
  ...
  for (int col = 0; col < FileManager::fNColumns; col++) {
    ...
    for (int row = 0; row < FileManager::fNRows; row++)
      temp[col][row] = FileManager::fData[col][row];
  }
  ...
  for (int i = 0; i < size; i++)
    FileManager::fDataNames[i] = tempName[i];
  FileManager::fDataNames[size] = columnName;

  for (int col = 0; col < FileManager::fNColumns - 1; col++)
    delete[] FileManager::fData[col];
  delete[] FileManager::fData;
  FileManager::fData = new double *[FileManager::fNColumns];
  for (int col = 0; col < FileManager::fNColumns; col++)
    FileManager::fData[col] = new double[FileManager::fNRows];

  for (int col = 0; col < size; col++)
    for (int row = 0; row < FileManager::fNRows; row++)
      fData[col][row] = temp[col][row];
  for (int row = 0; row < FileManager::fNRows; row++)
    fData[size][row] = column[row];
}
 ...
 * @param columnName
 */
void FileManager::addColumn(std::vector<double> &column,
...
double FileManager::max(const int column) const {
  ...
  for (int row = 1; row < fNRows; ++row) {
    if (fData[column][row] > max)
      max = fData[column][row];
  }
...
double FileManager::min(const int column) const {
  ...
  for (int row = 1; row < fNRows; ++row) {
    if (fData[column][row] < min)
      min = fData[column][row];
  }
...
/**
 * @brief Return unique values in a specific column as a dynamically allocated
 * array.
 *
 * @param column
...
void FileManager::print() const {
  for (int col = 0; col < FileManager::fNColumns; col++)
    std::cout << FileManager::fDataNames[col] << " ";
  std::cout << std::endl;
...
/**
 * @brief Overloads the file and writes in all currently stored data (comments
 * are not overloaded).
 *
 */
void FileManager::update() const {
  ...
  for (int i = 0; i < FileManager::fNCommentLines; i++)
    getline(reader, comments[i]);
  reader.close();
  ...
  for (int col = 0; col < FileManager::fNColumns; col++) {
    if (col != FileManager::fNColumns - 1)
      writer << FileManager::fDataNames[col] << " ";
    else
      writer << FileManager::fDataNames[col] << "\n";
  }
  ...
  for (int row = 0; row < FileManager::fNRows; row++) {
    for (int col = 0; col < FileManager::fNColumns; col++) {
      if (col != FileManager::fNColumns - 1)
        writer << FileManager::fData[col][row] << " ";
      else
        writer << FileManager::fData[col][row];
    }
    if (row != FileManager::fNRows - 1)
      writer << "\n";
  }
...
/**
 * @brief Initialize all data member with informations and content of a given
 * file.
 *
 * @param filePath
 */
void FileManager::open(const char *filePath) {
  ...
  FileManager::fExtension = FileManager::fFilePath.substr(
      FileManager::fFilePath.find_last_of(".") + 1);
...
/**
 * @brief Deletes all dynamically allocated variables and sets all data member
 * to default values.
 *
 */
...
void FileManager::uploadData() {
  ...
  std::string line;
  double val;
  for (int i = 0; i < FileManager::fNCommentLines + 1; i++)
    getline(streamer, line); // skip comments and column names
...
void FileManager::fillNamesArray() {
  ...
  std::string line, val;
  for (int i = 0; i < FileManager::fNCommentLines; i++)
    getline(streamer, line); // skip comments
...
/**
 * @brief Function to return the number of comment (or empty) lines in the
 * chosen file. Comment lines begin with "#".
 *
 * @return int
 */
