...
Projections2D::Projections2D(std::vector<Cluster> clusters)
    : fClusters(clusters),
      fCanvas(TCanvas("ProjectionCanvas", "ProjectionCanvas", 1280, 720)) {
  ...
  for (auto const &cluster : fClusters) {
    ...
    fClusterSizes[cluster.GetChipID()].push_back(cluster.GetClusterSize() *
                                                 1000);
  ...
  for (auto i = 0; i < N_CHIPS; i++) {
    fChipsHistos[i] =
        new TScatter(fClusterSizes[i].size(), fX[i].data(), fY[i].data(),
                     fColors[i].data(), fClusterSizes[i].data());
