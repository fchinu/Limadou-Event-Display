...
Geometry::Geometry() {
  ...
  std::unique_ptr<DetectorChip> dummyChip = std::make_unique<DetectorChip>(
      0, limadou::N_PIXELS_X, limadou::N_PIXELS_Y, limadou::PX_SIZE_X,
      limadou::PX_SIZE_Y, limadou::SIZE_Z);
  // DetectorChip* dummyChip = new DetectorChip(0, limadou::N_PIXELS_X,
  // limadou::N_PIXELS_Y, limadou::PX_SIZE_X, limadou::PX_SIZE_Y,
  // limadou::SIZE_Z);

  // create the detector chips
  for (unsigned imodule = 0; imodule < limadou::N_MODULES; imodule++) {

    fModules[imodule] = new DetectorModule(
        limadou::MODULE_IDS[imodule], limadou::CHIP_GAP_X, limadou::CHIP_GAP_Y,
        limadou::SIZE_Z, *dummyChip, limadou::CHIP_IDS);

    fModules[imodule]->SetPosition(
        0., 0., zOffsetModule + limadou::MODULE_POS_Z[imodule]);

    fChips[imodule] =
        std::array<std::array<DetectorChip *, limadou::N_PIXELS_X>,
                   limadou::N_PIXELS_Y>();

    for (unsigned ix = 0; ix < limadou::N_CHIPS_X; ix++) {
      for (unsigned iy = 0; iy < limadou::N_CHIPS_Y; iy++) {

        fChips[imodule][ix][iy] = new DetectorChip(
            limadou::CHIP_IDS[ix][iy], limadou::N_PIXELS_X, limadou::N_PIXELS_Y,
            limadou::PX_SIZE_X, limadou::PX_SIZE_Y, limadou::SIZE_Z);
        fChips[imodule][ix][iy]->SetPosition(fModules[imodule]->GetChipXPos(ix),
                                             fModules[imodule]->GetChipYPos(iy),
                                             fModules[imodule]->GetPosZ() +
                                                 0.1 * limadou::SIZE_Z);
      }
    }
  }
...
Geometry::Geometry(const Geometry &geometry) {
  for (unsigned imodule = 0; imodule < limadou::N_MODULES; imodule++) {
    ...
    for (unsigned ix = 0; ix < limadou::N_CHIPS_X; ix++) {
      for (unsigned iy = 0; iy < limadou::N_CHIPS_Y; iy++) {
        fChips[imodule][ix][iy] =
            new DetectorChip(*geometry.fChips[imodule][ix][iy]);
      }
    }
  }
}
...
DetectorChip *Geometry::GetChip(const unsigned moduleID,
                                const unsigned chipID) const {
  ...
  if (limadou::MODULE_IDS_MAP.count(moduleID)) {
    return fChips[limadou::MODULE_IDS_MAP[moduleID]][chipIndex.first]
                 [chipIndex.second];
  } else {
